...
class HrResignation(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string="Employé", default=_get_employee_id,
                                  help='Name of the employee for whom the request is creating')
    department_id = fields.Many2one('hr.department', string="Departement", related='employee_id.department_id',
                                    help='Department of the employee')
    joined_date = fields.Date(string="Date d'embauche", related='employee_id.start_date',
                              help='Joining date of the employee')
    expected_revealing_date = fields.Date(string="Date prévisionnelle de départ", required=True,
                                          help='Date on which he is revealing from the company')
    resign_confirm_date = fields.Date(string="Date éffective de départ", help='Date on which the request is confirmed')
    approved_revealing_date = fields.Date(string="Date d'approbation", help='The date approved for the releaving')
    reason_type= fields.Selection([('resignation', 'Démission'),('abort', 'Abandon de poste'),('lay_off', 'Licencement'),
                   ('end_contract','Fin de contrat'),('death', 'Décès'),('pension', 'Départ à la rétraite')],
                                  string="Type", required=True)
    reason = fields.Text(string="Raison", help='Specify reason for leaving the company')
